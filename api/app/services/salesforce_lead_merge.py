"""Salesforce Lead merge operations service.

Mirrors SalesforceMergeService (contacts) but targets the Lead sObject. The SOAP
merge() call DOES support Lead (alongside Account, Contact, Case) — the master
lead is kept, loser leads are deleted, and their child records (campaign members,
activities, …) are re-parented to the master. Max 2 records per call, so N losers
take ceil(N/2) sequential calls. Same public contract as the other merge services
so the CRM-agnostic run_merge drives it unchanged.
"""
...
class SalesforceLeadMergeService:
    """Service for merging Lead records in Salesforce (SOAP merge())."""
    ...
    async def merge_leads(self, master_id: str, merge_ids: list[str]) -> dict:
        """Merge leads into master via SOAP merge(), max 2 per call.
        ...
        async with httpx.AsyncClient() as client:
            for i in range(0, len(merge_ids), 2):
                ...
                if not ok:
                    errors.append(
                        f"Salesforce lead merge failed for {batch}: "
    ...
    def _merge_envelope(self, master_id: str, loser_ids: list[str]) -> str:
        """SOAP Partner-API merge() envelope for Leads. Blended winner fields are
        already applied via update_lead (REST PATCH), so masterRecord carries only
        type + Id here."""
        losers = "".join(
    ...
    async def update_lead(self, lead_id: str, properties: dict) -> dict:
        """Update a lead's fields (applies blended winner values before merge).

        Curated Contact-model keys map to Lead API field names. Unlike Contact,
        `company` maps to the writable Lead.Company text field. Beyond the curated
        set, raw fields the reviewer picked in the all-fields comparison pass through
        by their Lead API name (the review UI gates them to UPDATEABLE fields). `skip`
        are model-only keys with no Lead column, so we never send an unknown column
        (INVALID_FIELD).
        """
        field_mapping = {
            "email": "Email",
            ...
            "last_name": "LastName",
            "phone": "Phone",
            "company": "Company",  # Lead.Company IS a writable text field
            "job_title": "Title",
        }
        skip = {"created_at", "updated_at", "association_count", "id", "full_name"}

        sf_properties = {}
        # 1) curated snake keys -> Lead field names (exact snake match).
        for key, value in properties.items():
            if key in field_mapping and value not in (None, ""):
                sf_properties[field_mapping[key]] = value
        # 2) raw picked fields (Lead API names) pass through and OVERRIDE the curated
        # default for the same field — step 2 runs after step 1, so it's
        # order-independent. Skip curated snake keys (handled above) and model-only keys.
        for key, value in properties.items():
        ...
        async with httpx.AsyncClient() as client:
            ...
            return {
                "success": False,
                "error": f"Salesforce lead update failed: {response.status_code} - {response.text}",
            }
    ...
    async def merge_duplicate_set(
        ...
        """Merge a complete duplicate set of leads.
        ...
        if blended_properties:
            update_result = await self.update_lead(winner_id, blended_properties)
        ...
        merge_result = await self.merge_leads(winner_id, loser_ids)
